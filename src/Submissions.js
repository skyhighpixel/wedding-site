  ...
  useEffect(() => {
    $.ajax({
      ...
      success: function (response) {
        ...
        console.log("RES", pluck(response.data, "contact").join(","));
        setSubmissions(
          response.data
            .filter((submissions) => {
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Guest</th>
            <th>
              Attending (Yes: {attendingCount?.length} No:{" "}
              {notAttendingCount?.length})
            </th>
            <th>Dietery</th>
            <th>Songs</th>
            <th>Email</th>
                ...
                return (
                  <tr
                    ...
                    style={{
                      backgroundColor:
                        i % 2
                          ? "rgba(var(--theme-sage-rgb), 0.1)"
                          : "rgba(var(--theme-sage-rgb), 0.3)",
                    }}
                  >
